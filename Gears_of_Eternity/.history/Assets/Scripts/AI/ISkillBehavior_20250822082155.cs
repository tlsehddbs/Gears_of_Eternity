...
public class PassiveTurretBarrageSkill : ISkillBehavior
{
    ...
    private const float DefaultRadius = 2.5f;  // AoE 기본 반경
    private const float DefaultDeathRad = 3.0f;  // 자폭 기본 반경 (※ 아래 주석 참조)
    ...
    private IEnumerator FireLoop(UnitCombatFSM caster, SkillEffect effect)
    {
        ...
        // AoE 반경 결정(공격/자폭에 동일 반경을 사용할지 분리할지 기획에 따라 달라짐)
        float shotRadius = (effect != null && effect.skillValue > 0f) ? effect.skillValue : DefaultRadius;
        float deathRadius = DefaultDeathRad; // ※ 확실하지 않음: 자폭 반경 별도 값이 없으니 기본값 사용.
                                             //    동일 반경을 원하면 shotRadius로 바꿔도 OK.
        ...
        while (caster != null)
        {
            ...
            if (!caster.IsAlive())
            {
                SelfDestruct(caster, deathRadius);
                break;
            }
            ...
            {
                fireTimer = 0f;
                FireOnce(caster, effect, shotRadius);
            }
