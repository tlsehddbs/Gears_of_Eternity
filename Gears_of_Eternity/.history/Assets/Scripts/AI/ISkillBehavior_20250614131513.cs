...
public class BleedBurstSkill : ISkillBehavior
{
    ...
    private const float bleedDurationBase = 3f;
    private const float bleedStackDuration = 1f;
    private const float bleedPercentPerSec = 0.1f;
    ...
    public void Execute(UnitCombatFSM caster, UnitCombatFSM target, SkillEffect effect)
    {
        caster.StartCoroutine(ExecuteHits(caster));
        caster.skillTimer = 0f;
    }
    public void Remove(UnitCombatFSM caster, SkillEffect effect) { }
    ...
    private IEnumerator ExecuteHits(UnitCombatFSM caster, SkillEffect effect)
    {
        ...
        for (int i = 0; i < maxHits; i++)
        {
            ...
            if (target != null)
            {
                float damage = caster.stats.attack * effect.skillValue + fixedDamage;
                target.TakeDamage(damage);
                ApplyBleed(target);
            }
            yield return new WaitForSeconds(hitDelay);
    ...
    private void ApplyBleed(UnitCombatFSM target)
    {
        if (target.bleedEffect == null)
        {
            target.bleedEffect = new BleedEffect(target, bleedDurationBase, bleedPercentPerSec);
        }
        else
        {
            target.bleedEffect.Refresh(bleedStackDuration);
        }
    }
}