...
public class ConeTripleHitSkill : ISkillBehavior
{
    ...
    private const float RangeMultiplier = 1.8f;
    private const float damageMultplier = 0.9f;

    public bool ShouldTrigger(UnitCombatFSM caster, SkillEffect effect)
    {
        return caster.FindEnemiesInCone(Angle, RangeMultiplier).Count > 0;
    }

    public UnitCombatFSM FindTarget(UnitCombatFSM caster, UnitCombatFSM target, SkillEffect effect)
    {
    ...
    private IEnumerator ExecuteTripleHit(UnitCombatFSM caster, SkillEffect effect)
    {
        for (int i = 0; i < hitCount; i++)
        {
            var target = caster.FindEnemiesInCone(Angle, RangeMultiplier);
            foreach (var enemy in targets)
            {
                float damage = caster.stats.attack * damageMultplier;
                enemy.TakeDamage(damage);
