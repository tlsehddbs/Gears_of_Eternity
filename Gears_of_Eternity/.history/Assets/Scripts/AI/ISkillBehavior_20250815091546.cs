...
public class HeatReactiveMarkSkill : ISkillBehavior
{
    ...
    private IEnumerator ApplyMarkAndExplode(UnitCombatFSM caster, UnitCombatFSM target, SkillEffect effect)
    {
        ...
        //폭발: 주변 적에게 데미지
        ApplyAoEAroundTarget_ListScan(caster, target, baseAtk, AoERadius); // ★변경: 본체 제외 로직 포함
    ...
    private void ApplyAoEAroundTarget_ListScan(UnitCombatFSM caster, UnitCombatFSM center, float baseAtk, float radius)
    {
        if (caster == null || center == null) return;

        // ★변경: refUnit은 'caster'를 넣어야 적군만 반환됩니다.
        ...
        foreach (var enemy in enemies)
        {
            ...
            // ★변경: 본체는 AoE 대상에서 제외 (100% 피해를 이미 받았기 때문)
