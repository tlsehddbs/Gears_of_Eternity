...
{
    private static readonly Dictionary<UnitCombatFSM, Coroutine> routines = new();
    ...
    public void Execute(UnitCombatFSM caster, UnitCombatFSM target, SkillEffect effect)
    {
        if (routines.ContainsKey(caster)) return;

        Coroutine routine = caster.StartCoroutine(GrowRoutine(caster, effect));
        routines[caster] = routine;
    }

    public void Remove(UnitCombatFSM caster, SkillEffect effect)
    {
        if (routines.TryGetValue(caster, out var routine))
        {
            caster.StopCoroutine(routine);
            routines.Remove(caster);
        }

        // 총 누적된 버프 제거
        float total = effect.skillValue * effect.skillMaxStack;
        caster.ApplyBuff(effect.buffStat, total, 0f, effect.isPercent); // 0초 = 즉시 해제
    }
    ...
    private IEnumerator GrowRoutine(UnitCombatFSM caster, SkillEffect effect)
    {
        ...
        {
            yield return new WaitForSeconds(effect.skillDuration);
            stack++;
            caster.ApplyBuff(effect.buffStat, effect.skillValue, -1f, effect.isPercent);
